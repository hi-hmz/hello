...
def receive_coordinate():
    while True:
        ...
        canvas1.create_oval(rres[0], rres[1], rres[2], rres[3], fill='white')
        if len(rres) == 5:
            showinfo(title='提示', message='白方胜利,游戏结束')
            win.destroy()
        game_map[rres[4]][rres[5]] = 'white'
...
def send_coordinate(message):
    udp_client_socket.sendto(message.encode('utf8'), ('192.168.1.8', 3765))
...
if __name__ == '__main__':
    ...
    udp_client_socket.bind(('192.168.1.8', 3766))
    # 生成对弈窗口
    win = Tk()
    win.title('五子棋黑')
